...
## Зона E — уровень 23, хэш `0x8689AC71` («The City of Sarn» / `1_3_1`)

    IngameState = 0x5F4FC562810   (тот же запуск, что зона D)
    Data        = 0x5F61DF87800

Снималась ради двух вещей — портала и подтверждения игрока.

### `LocalPlayer` подтверждён СОДЕРЖИМЫМ

`RefLive --as Entity 0x5F61DF95B00` (адрес взят из `+0x970`) — парсер эталона вычитал живого
персонажа:

    BoundsCenterPos  X:7308,696 Y:5004,348 Z:-378,61426
    Buffs            18: Discipline Aura, Grace Aura, Haste Aura, Precision Aura,
                     One Step Ahead (trickster_nearby_enemy_action_speed), ...
    Components       Positioned, Stats, Pathfinding, Buffs, Life, Animated — с адресами

То есть по `+0x970` действительно лежит игрок, а не совпавшее число. Открытым оставалось не это,
а какой из двух адресов (`+0x970` или `+0x10E8`) — само поле.

> **Поправка от обследования 2026‑09‑16** (см. [survey-2026-09-16.md](survey-2026-09-16.md)).
> `+0x10E8` **опознан**, а не остался вторым кандидатом: это поле `+0x30` подобъекта, который
> начинается по `Data+0x10B8` и несёт там СОБСТВЕННУЮ vtable; `EnvironmentData` (`+0x1110`) —
> поле `+0x58` того же подобъекта. Значит `+0x10E8` — чужая ссылка на игрока, а не наше поле.
> Статус этого числа: **замерено на живом клиенте, НЕ переподтверждено.**
> Отдельно ловить состояние, где эти два адреса разойдутся, больше незачем.

Побочно это закрывает вопрос о следующем слое: **эталон разбирает сущности и отдаёт имена и
адреса компонентов**, значит `EntityOffsets` мерится обычными тремя шагами, а не разностью.

### `TownPortals` — попытка не удалась, и вот что именно установлено

Портал был открыт в этой зоне по просьбе. Установлено:

1. **Старые числа мертвы.** `0x4A0`..`0x4C8` — сплошные нули при открытом портале. Зашитые в
   `IngameData.TownPortals` смещения `0x4B4`/`0x4BC` не адресуют ничего.
2. **Эталон тут не помощник.** При открытом портале он отдаёт `TownPortals: 0`. Это тот же
   симптом, что `LabyrinthData = null` в Лабиринте. Других членов, которыми он моделирует
   порталы, у него нет (`ForkProbe --grepmember Portal` — ровно одна строка).
3. **Разность в окне `0x2000` не поймала список.** Между снимками «портала нет» и «портал есть»
   (одна зона, одна база) изменилось 19 qword из 1024, и ни один вектор не приобрёл элемент.
   Единственный переход «ноль → указатель» — тройка по `+0x90`, но она пуста в обоих состояниях
   (`First == Last`) и переезжает сама по себе: через пару минут она уже
   `0x5F50E3E3F70` с ёмкостью `0x30`, всё так же пустая.

Попытка была повторена во ВТОРОЙ зоне — «The Sewers» (`1_3_10_1`, уровень 26, хэш `0x6458CB8`,
`Data 0x5F55990D800`) — и уже окном `0x10000`, самым широким, какое читается за один раз.
Результат тот же: изменилось 18 qword из 8192, **ни один вектор не приобрёл элемент**. Оба
перехода «ноль → указатель» — векторы по `+0x90` и `+0xA48`, пустые в обоих состояниях
(`First == Last`), переезжающие сами между чтениями.

**Итог: это не «пока не замерено», а «здесь этого нет».** На этой сборке список порталов не
лежит в первых `0x10000` байтах `IngameData`. Закрывать его надо другим приёмом: сперва найти
сущность портала (эталон разбирает сущности верно — `RefLive --as Entity` на ближайших выдал
`Metadata/Monsters/Grappler/Grappler@26` «Undying Grappler», `Metadata/Pet/ScientistLabRat`,
`Metadata/MiscellaneousObjects/Doodad`), а затем искать, кто на неё ссылается.
...
1. **Не покрыты состояния**: город/убежище, загрузочный экран, выбор персонажа, смерть.
   ...
   **Снято обследованием 2026‑09‑16**: `+0x10E8` опознан как поле `+0x30` подобъекта с
   собственной vtable по `Data+0x10B8` (тому же, чьё поле `+0x58` — `EnvironmentData`), то есть
   это чужая ссылка на игрока, а не второй кандидат на наше поле. Ловить такое состояние ради
   этого вопроса не нужно. Само опознание — замерено на живом клиенте, не переподтверждено.
