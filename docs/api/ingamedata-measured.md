...
То есть по `+0x970` действительно лежит игрок, а не совпавшее число. Открытым остаётся не это,
а какой из двух адресов (`+0x970` или `+0x10E8`) — само поле; их разводит только состояние,
где игрока нет.
