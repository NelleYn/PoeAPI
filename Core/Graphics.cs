...
/// <summary>
/// High-level drawing facade over the DirectX 11 renderer. Provides text, line, box, frame and
...
public class Graphics
{
    ...
    /// <summary>Creates the drawing facade over the supplied DX11 renderer and core settings.</summary>
    ...
    /// <summary>The underlying low-level DX11 renderer.</summary>
    public DX11 LowLevel { get; }

    /// <summary>Whether the ImGui overlay is currently in its transparent (click-through) state.</summary>
    public bool TransparentState => ImGuiRender.TransparentState;

    /// <summary>The font currently selected in the core settings.</summary>
    public FontContainer Font => ImGuiRender.fonts[_settings.Font];

    /// <summary>The most recently used font.</summary>
    public FontContainer LastFont => ImGuiRender.CurrentFont;

    /// <summary>Draws text at the given position and returns the rendered text size. Several overloads vary by color, font, height and alignment.</summary>
    ...
    /// <summary>Measures the rendered size of the given text.</summary>
    ...
    /// <summary>Draws a line between two points with the given width and color.</summary>
    ...
    /// <summary>Draws a filled rectangle (box). Overloads accept a rectangle or corner points.</summary>
    ...
    /// <summary>Draws a sprite image into the given rectangle. Overloads accept a color, UV region or an atlas texture.</summary>
    ...
    /// <summary>Draws an image through the ImGui render path (rather than the sprite renderer).</summary>
    ...
    /// <summary>Loads a PNG image for later drawing. When <paramref name="textures"/> is true the path is resolved under the <c>textures</c> folder. Returns whether the image loaded.</summary>
    public bool InitImage(string name, bool textures = true)
    {
        return SpritesRender.LoadPng(textures ? $"textures/{name}" : name)!=null;
    }

    /// <summary>Releases a previously loaded texture.</summary>
